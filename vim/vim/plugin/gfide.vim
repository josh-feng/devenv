"=============================================================================
" File:        gfide.vim (project.vim + tagbar.vim)
" Author:      Aric Blumer (Aric.Blumer at aricvim@charter.net)
"              Josh Feng <jui-hsuan.feng@globalfoundries.com> handling Tagbar
"              Jan Larres <jan@majutsushi.net> Tagbar
" Last Change: Fri 13 Oct 2006 09:47:08 AM EDT
" Version:     0.02 (1.4.1a + 2.4.1)
...
" call confirm('Hide Tagbar.', "&OK", 1)

scriptencoding utf-8

if exists('loaded_ide') || &cp
  finish
endif
let loaded_ide = 1
...
" Initialize " {{{
" IDE
if !exists('g:ide_width')
    let g:ide_width = 24
endif
if !exists('g:ide_incre')
    let g:ide_incre = 100         " IDE Window width increment
endif
if !exists('g:ide_flags')
    let g:ide_flags = 'imstb'     " IDE default flags for everything else
endif
...
if !exists('g:tagbar_width')
    let g:tagbar_width = g:ide_width
endif
...
function! s:IDE(filename) " {{{
    " Initialization {{{
    if exists("g:ide_running")
        if strlen(a:filename) != 0
            call confirm('IDE loaded; ignoring "'.a:filename."\".\n".'See ":help ide-invoking".', "&OK", 1)
        endif
        let filename = bufname(g:ide_running)

        " Toggle Close the IDE window
        if bufwinnr(g:ide_running) != -1
            let tbwn = bufwinnr("__Tagbar__")
            if tbwn > 0
                exec tbwn."wincmd w" | hide
            endif
            exec bufwinnr(g:ide_running)."wincmd w" | hide
            let g:ide_lastwn = bufwinnr(g:ide_lastbn)
            exec g:ide_lastwn."wincmd w"
            return
        endif

        " update the last window number
        if (bufnr('%') != g:ide_running) && (bufnr('%') != bufnr("__Tagbar__"))
            let g:ide_lastwn = winnr()
        endif
        ...
        " initialize the last window number
        let g:ide_lastwn = winnr()
        let g:ide_lastbn = bufnr('%')
    endif

    " Open the IDE Window
    let tbbn = bufnr("__Tagbar__")
    if tbbn == -1 " Open the Tagbar
        TagbarOpen
        let tbbn = bufnr("__Tagbar__")
    end
    let tbwn = bufwinnr("__Tagbar__")
    ...
    if exists("g:ide_running") && (g:ide_running != -1)
        exec 'silent vsp | b '.g:ide_running
    else
        exec 'silent vnew '.filename
    end
    silent! wincmd L
    exec 'bel split | b '.tbbn.' | wincmd k'
    unlet tbbn
    unlet tbwn
    ...
    " Process the flags
    let b:ide_cd_cmd = 'cd'
    if match(g:ide_flags, '\Cl') != -1
        let b:ide_cd_cmd = 'lcd'
    endif

    let b:ide_locate_cmd = '' " 'silent! wincmd L'
    let b:ide_resize_cmd = 'exec ''vertical resize ''.g:ide_width'

    if match(g:ide_flags, '\CF') != -1 " Floating: set the resize commands to nothing
        let b:ide_locate_cmd = ''
        let b:ide_resize_cmd = ''
    endif
    " }}}

    " IdeFoldText() {{{
    "   The foldtext function for displaying just the description.
    function! IdeFoldText()
    ...
    function! s:DoSetup()
        ...
        setlocal foldtext=IdeFoldText() nobuflisted nowrap
        setlocal winwidth=1
        if match(g:ide_flags, '\Cn') != -1
            setlocal number
        endif
    endfunction "}}}
    ...
    call s:DoSetup()
    " Syntax Stuff {{{
    if match(g:ide_flags, '\Cs')!=-1 && has('syntax') && exists('g:syntax_on') && !has('syntax_items')
    ...
    function! s:DoSetupAndSplit()
        ...
        if (bufwinnr("__Tagbar__") == winnr('#')) || (bufnr('#') == g:ide_running)
            exec g:ide_lastwn.'wincmd w'
        else
            silent! wincmd p
        endif
    ...
    "   Same as above but ensure that the IDE window is the current
    "   window.  Only called from an autocommand
    function! s:DoSetupAndSplit_au()
        if winbufnr(0) != g:ide_running
            return
        endif
        call s:DoSetup()                " Ensure that all the settings are right
        if winbufnr(2) == -1            " We're the only window right now.
            exec 'silent vertical split | bnext'
            if bufnr('%') == g:ide_running
                enew
            endif
            if bufnr('%') == g:ide_lastbn | bnext | bprev | bnext | endif
            wincmd p " Go back to the IDE Window and ensure it is the right width
            exec b:ide_locate_cmd
            exec b:ide_resize_cmd
        elseif (winnr() != 1)
            exec b:ide_locate_cmd
            exec b:ide_resize_cmd
        endif
    ...
    function! s:RecordPrevBuffer_au()
        ...
        if (bn != g:ide_running) && (bn != bufnr("__Tagbar__"))
            let g:ide_lastbn = bn
        endif
        unlet bn
    endfunction "}}}
    ...
    function! s:RecursivelyConstructDirectives(lineno)
        ...
        if home != ''
            if (foldlevel(foldlineno) == 1) && !s:IsAbsolutePath(home)
                call confirm('Outermost IDE Fold must have absolute path!  Or perhaps the path does not exist.', "&OK", 1)
        ...
        if c_d != ''
            if (foldlevel(foldlineno) == 1) && !s:IsAbsolutePath(c_d)
                call confirm('Outermost IDE Fold must have absolute CD path!  Or perhaps the path does not exist.', "&OK", 1)
    ...
    function! s:OpenEntry2(line, infoline, fname, editcmd)
        ...
        if home=='/'
            echoerr 'IDE structure error. Check your syntax.'
            return
        endif
        "Save the cd command
        let cd_cmd = b:ide_cd_cmd
        if a:editcmd[0] != '' " If editcmd is '', then just set up the environment in the IDE Window
        ...
        else " only happens in the IDE File
    ...
    function! s:CreateEntriesFromDir(recursive)
        ...
        let home=''
        let filter='*'
        if (match(g:ide_flags, '\Cb') != -1) && has('browse')
    ...
    function! s:RefreshEntriesFromDir(recursive)
        ...
        if just_a_fold == 0
            ...
            if !isdirectory(glob(home))
                ...
                if (match(flags, '\Ct') != -1) || ((match(g:ide_flags, '\CT') == -1) && (match(flags, '\CT') == -1))
    ...
    "   Ensure that the IDE window is on the left of the window and has
    "   the correct size. Only called from an autocommand
    function! s:DoEnsurePlacementSize_au()
        if (winbufnr(0) != g:ide_running) || (winnr() != 1)
            if exists("g:ide_doinghelp")
                if g:ide_doinghelp > 0
                    let g:ide_doinghelp = g:ide_doinghelp - 1
                    return
                endif
                unlet g:ide_doinghelp
                return
            endif
            exec b:ide_locate_cmd
        endif
    ...
    function! s:LoadAll(recurse, line)
        ...
        call IDE_ForEach(a:recurse, line('.'), "*<SID>SpawnExec", 0, '^\(.*l\)\@!')
    ...
    function! s:WipeAll(recurse, line)
        ...
        call IDE_ForEach(a:recurse, line('.'), "<SID>SpawnExec", 0, '^\(.*w\)\@!')
        delfunction s:SpawnExec
    ...
    function! s:LoadAllSplit(recurse, line)
        let b:loadcount=0
        function! s:SpawnExec(infoline, fname, lineno, data)
            let winNr = winnr() "get IDEWindow number
    ...
    function! s:GrepAll(recurse, lineno, pattern)
        ...
        let fnames=IDE_GetAllFnames(a:recurse, a:lineno, ' ')
        unlet b:escape_spaces
        cclose " Make sure grep window is closed
        call s:DoSetupAndSplit()
        if match(g:ide_flags, '\Cv') == -1
            ...
            if v:shell_error != 0
                ...
                " Josh
                exec bufwinnr(g:ide_running).'wincmd w | hide'
                if bufwinnr("__Tagbar__")
                    exec bufwinnr("__Tagbar__")."wincmd w" | hide
                endif
                copen
            endif
        else
    ...
    " IDE_GetAllFnames(recurse, lineno, separator) {{{
    "   Grep all files in a project, optionally recursively
    function! IDE_GetAllFnames(recurse, lineno, separator)
        let b:fnamelist=''
        function! s:SpawnExec(home, c_d, fname, lineno, data)
        ...
        call IDE_ForEach(a:recurse, line('.'), "<SID>SpawnExec", a:separator, '')
        delfunction s:SpawnExec
    ...
    " IDE_ForEach(recurse, lineno, cmd, data, match) {{{
    "   Grep all files in a project, optionally recursively
    function! IDE_ForEach(recurse, lineno, cmd, data, match)
        ...
        if (flags == '') || (a:match=='') || (match(flags, a:match) != -1)
            call s:IDE_ForEachR(a:recurse, lineno, info, a:cmd, a:data, a:match)
        endif
    endfunction
    function! s:IDE_ForEachR(recurse, lineno, info, cmd, data, match)
            ...
            if curline =~ '{'
                if a:recurse
                    ...
                    if (flags == '') || (a:match=='') || (match(flags, a:match) != -1)
                        ...
                        let lineno=s:IDE_ForEachR(1, lineno+1,
    ...
    function! s:SpawnAll(recurse, number)
        echo | if exists("g:proj_run_fold".a:number)
            if g:proj_run_fold{a:number}[0] == '*'
                ...
                call IDE_ForEach(a:recurse, line('.'), "<SID>SpawnExec", a:number, '.')
                delfunction s:SpawnExec
            else
                ...
                let b:escape_spaces=1
                let fnames=IDE_GetAllFnames(a:recurse, line('.'), ' ')
                unlet b:escape_spaces
    ...
    if !exists("g:ide_running")
        ...
        " s:DoIDEOnly(void) {{{
        "   Make the file window the only one. TODO
        function! s:DoIDEOnly()
            if winbufnr(0) != g:ide_running
                let lzsave = &lz
                set lz
                only
        ...
        cnoremap <buffer> help let g:ide_doinghelp = 1<CR>:help
        nnoremap <buffer> <F1> :let g:ide_doinghelp = 1<CR><F1>
        ...
        if match(g:ide_flags, '\Cm') != -1
            if !hasmapto('<Plug>IDEOnly')
                nmap <silent> <unique> <C-W>o <Plug>IDEOnly
        ...
        let bufname=escape(substitute(expand('%:p', 0), '\\', '/', 'g'), ' ')
        exec 'au BufWipeout '.bufname.' au! * '.bufname
        exec 'au BufWipeout '.bufname.' unlet g:ide_running'
        ...
        " }}}
        setlocal buflisted
        let g:ide_running = bufnr(bufname.'\>')
        if g:ide_running == -1
            call confirm('IDE/Vim error. Please Enter :IDE again and report this bug.', "&OK", 1)
            unlet g:ide_running
        endif
...
finish
" Modeline
" vim: ts=8 sw=4 sts=4 et foldenable foldmethod=marker foldmarker={{{,}}} foldlevel=1:
